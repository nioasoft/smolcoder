...
Detection builds a list of addresses where a server could be, then asks each one what it is. The answer decides the backend, never the port: an address that returns Ollama's model list (`/api/tags`) is Ollama, one that returns LM Studio's catalog (`/api/v1/models`, or the older listings) is LM Studio, and one whose `/health` reports an `engine_pool` is oMLX. oMLX wants an API key on every other endpoint: for a server on this computer it is read from `~/.omlx/settings.json`, and `OMLX_API_KEY` supplies one for any address. The settings key is never sent off this computer. A `/health` that reports `mtp_enabled` and `generation_mode` is MTPLX, which shares port 8000 with oMLX by default; its own port is read from the app's settings, and `MTPLX_API_KEY` is sent when set. Both requests go out together, so an address with nothing behind it costs one timeout.
